...
def process_port_udp(in_queue, out_queue, settings):
    try:
        ...
        udp_socket.bind((settings['udp']['bind_ip'], settings['udp']['bind_port']))
        ...
        # string parsing
            ...
            while not state.stop:
                ...
                for symbol in symbols:
                    # splitter detected
                    symbol = symbol.to_bytes(1, byteorder='big') # convert to bytes
                    if symbol in b'\r\n':
                        if r_state != 1:
                            data = data.strip()
                            if data:
...
class GraphsView(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.settings_frame.push_button_open.clicked.connect(    self.on_open_port_serial)   
        self.settings_frame.push_button_open_udp.clicked.connect(self.on_open_port_udp   )
        self.settings_frame.push_button_clear.clicked.connect(   self.on_clear_graphs    )
        self.settings_frame.push_button_pause.clicked.connect(   self.pause              )
        self.settings_frame.check_box_xy_mode.toggled.connect(   self.xy_mode_changed    )
        self.curves = {}
        self.results = {}
